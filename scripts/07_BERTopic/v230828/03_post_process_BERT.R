...
# Load Topics
topics <- read.csv(here("scripts", "07_BERTopic", "v230828", "BERT_data", "topic_info.csv"),
                   header = T)

# Load Doc Info
doc_info <- read.csv(here("scripts", "07_BERTopic", "v230828", "BERT_data", "doc_info.csv"),
                     header = T,
...
topic_probs <- read.csv(here("scripts", "07_BERTopic", "v230828", "BERT_data", "probs.csv"),
                        header = T,
                        colClasses = c("BusinessShortNumber" = "character"),
...
top_topics <- topic_probs %>%
  ...
  mutate(prob_1 = case_when(prob_1 < cutoff ~ NA,
                            prob_1 >= cutoff ~ prob_1)) %>%
  mutate(prob_2 = case_when(prob_2 < cutoff ~ NA,
                             prob_2 >= cutoff ~ prob_2)) %>% 
...
# Create cutoff for main topic
top_topics_long <- top_topics_long %>% 
  mutate(topic_value = case_when(is.na(prob_1) == T ~ NA,
                                 is.na(prob_1) == F ~ topic_value))

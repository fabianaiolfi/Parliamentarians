...
topic_probs$diff_max_3 <- apply(topic_probs[, paste0("V", 1:140)], 1, function(row) {
  sorted_values <- sort(row, decreasing = TRUE)
  return(sorted_values[1] - sorted_values[3])
})

# Get 1st probability (max)
topic_probs$prob_1 <- do.call(pmax, topic_probs[, paste0("V", 1:140)])

# Get 2nd highest probability
topic_probs$prob_2 <- apply(topic_probs[, paste0("V", 1:140)], 1, function(x) x[order(x, decreasing = TRUE)[2]])

# Get 3rd highest probability
topic_probs$prob_3 <- apply(topic_probs[, paste0("V", 1:140)], 1, function(x) x[order(x, decreasing = TRUE)[3]])
...
topic_probs$top_3_topics <- apply(topic_probs[, paste0("V", 1:140)], 1, function(row) {

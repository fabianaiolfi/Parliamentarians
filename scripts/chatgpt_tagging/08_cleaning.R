...
temp_df <- tokens(all_businesses$chatgpt_tags_clean, remove_punct = TRUE, remove_numbers = TRUE, remove_symbol = TRUE)
temp_df <- tokens_remove(temp_df, pattern = c(stopwords("de"), custom_stopwords))
...
# short eval - perhaps more stopwords based on this?
...
words <- unlist(strsplit(all_tags, "\\s"))
words <- gsub('\n', '', words) # remove \n
...
all_businesses <- all_businesses %>% mutate_at(vars(chatgpt_tags), ~ gsub("\n", " ", .))

# Add </s> at end of strings
all_businesses <- all_businesses %>% mutate_at(vars(chatgpt_tags), ~ paste0(., "</s>"))

all_businesses$chatgpt_tags[1]

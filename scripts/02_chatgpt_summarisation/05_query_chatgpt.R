...
chatgpt_output_df <- data.frame(id = character(0), chatgpt_summary = character(0))

# ChatGPT API Query in sleep loop to prevent reaching tokens-per-minute limit of 10'000
for(i in 1:nrow(all_businesses_long)) {
  chatgpt_output_summary <- chatgpt(prompt_role_var = all_businesses_long$role[i],
  ...
  chatgpt_output_summary <- do.call(
    ...
    Map(data.frame,
        ...
        chatgpt_summary = chatgpt_output_summary[[1]][["chatgpt_content"]]))

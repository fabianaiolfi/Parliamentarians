...
finanzwesen <- df %>% dplyr::filter(main_tag == "Finanzwesen")


# Create vector of BusinessShortNumbers
finanzwesen_bsn <- finanzwesen %>% select(BusinessShortNumber)
...
groups <- cutree(final_clust, k = 24)

# Append cluster labels to df
finanzwesen <- cbind(finanzwesen, cluster = groups)
#all_businesses_cluster <- all_businesses

finanzwesen %>% 
  dplyr::filter(cluster == floor(runif(1, min = 1, max = 21))) %>%

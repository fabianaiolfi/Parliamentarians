...
const fetchAndSetRelatedData = (personId) => {
  ...
  if (WorryStatement[personId]) {
    ...
    // Set available topics for the a-select component
    availableTopics.value = Object.keys(personData);

    // Initially, no topic is selected, so no statement is shown in the a-card
    selectedMainTopic.value = null;
    selectedTopic.value = null;
    cardStatement.value = ''; // Reset the card statement
  } else {
    // Reset or handle case where personId doesn't have an entry in WorryStatement
...
const handleMainTopicChange = (topicValue) => {
  ...
  // Assuming 'selectedStatements' holds the statements for all topics
  // for the currently selected person
  if (selectedStatements.value && selectedStatements.value[topicValue]) {
    cardStatement.value = selectedStatements.value[topicValue];
  } else {
    // Handle cases where there's no statement for the selected topic
...
watch(selectedMainTopic, (newTopic) => {
  if (newTopic) {
    // Update the statement for the a-card
    const statement = selectedStatements.value[newTopic];
    if (statement) {
      // Assuming you have a reactive variable to hold the statement for the a-card
      cardStatement.value = statement;
    } else {
      // If there's no statement for the selected topic, clear the a-card
      cardStatement.value = '';
    }

    // TODO: Update table data based on the new topic
    // For example, you might fetch new data or filter existing data
    // This part depends on how your table data needs to react to topic changes
  } else {
    // If no topic is selected, clear the data in the a-card and table
    cardStatement.value = '';
    // TODO: Clear or reset the table data
  }
});
...
<div style="background: #F5F5F5; padding: 0px; display: flex; flex-direction: column;" v-if="selectedPerson">
  ...
  <div style="width: 100%;"> <!--  Adjust the width to your liking -->
    ...
    <!-- <div v-if="selectedMainTopic !== 'Alle Themen' && cardStatement.valueOf"> -->
    <div>

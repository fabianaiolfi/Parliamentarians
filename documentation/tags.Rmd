...
business_legislative_period_51$InitialSituation_clean <- paste(business_legislative_period_51$Title,
                                                               business_legislative_period_51$InitialSituation_clean)
```
...
```{r}
business_legislative_period_51 %>% select(BusinessShortNumber, TagNames) %>% head()
```

There are a total of 27 different tags:
```{r}
...
```{r}
business_tags_plot <- business_legislative_period_51 %>% 
...
### Compare Clusters with Tags
```{r}
topic_check <- business_legislative_period_51 %>% select(TagNames)
